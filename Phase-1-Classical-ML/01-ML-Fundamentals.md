...
Does this actually require ML? Could a simple rule/heuristic solve it just as well?
Do I have labeled data (correct answers)? → Supervised. No labels, but want structure? → Unsupervised.
If supervised: am I predicting a number or a category? → Regression or Classification.
What are my X (features) and y (target)?
Worked Examples
